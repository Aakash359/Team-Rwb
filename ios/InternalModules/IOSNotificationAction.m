...
NSString *const IOSNotificationActionReceived = @"NotificationActionReceived";
...
@implementation IOSNotificationAction
...
- (void)setBridge:(RCTBridge *)bridge
{
  ...
  [[NSNotificationCenter defaultCenter] addObserver:self
                                           selector:@selector(handleNotificationActionReceived:)
                                               name:IOSNotificationActionReceived
                                             object:nil];
  NSDictionary *lastActionInfo = [IOSNotificationActionManager sharedInstance].lastActionInfo;
  if(lastActionInfo != nil) {
    [[NSNotificationCenter defaultCenter] postNotificationName:IOSNotificationActionReceived
                                                        object:self
                                                      userInfo:lastActionInfo];
    [IOSNotificationActionManager sharedInstance].lastActionInfo = nil;
  }
}
...
{
  void (^completionHandler)() = [IOSNotificationActionManager sharedInstance].lastCompletionHandler;
  if(completionHandler != nil) {
    completionHandler();
    [IOSNotificationActionManager sharedInstance].lastCompletionHandler = nil;
  }
}
...
+ (void)emitNotificationActionForIdentifier:(NSString *)identifier source:(NSString *)source responseInfo:(NSDictionary *)responseInfo userInfo:(NSDictionary *)userInfo completionHandler:(void (^)())completionHandler
{
  ...
  [[NSNotificationCenter defaultCenter] postNotificationName:IOSNotificationActionReceived
                                                      object:self
                                                    userInfo:info];
  [IOSNotificationActionManager sharedInstance].lastActionInfo = info;
  [IOSNotificationActionManager sharedInstance].lastCompletionHandler = completionHandler;
}
